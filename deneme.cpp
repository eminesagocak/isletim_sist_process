...
//#include <windows.h>  Windows API işlevleri için bu kullanılır.

int main() {
    pid_t pid;
    // child proses oluştururken
    pid = fork();
    ...
    if (pid < 0) {
        ...
        //excelp komutu çocuk presess içinde çalıştırıyor 
        execlp("cat", "cat", "dosya.txt", NULL);
        //execlp başarılı ise buraya gelmez
        perror("execlp");
        exit(1);
    } else {
